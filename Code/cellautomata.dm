/obj/team/proc/process()
	if (src.base)
		...
		while(locate(text("landmark*CTF-supply-[]", src.base)))
			...
			I.registered = text("[]", uppertext((src.colour ? src.colour : "rogue")))
			...
			if (src.colour)
				F.icon_state = text("flag_[]", src.colour)
				F.name = text("flag- '[] Team's Flag'", uppertext(src.colour))
			else
				F.name = "flag- 'NEUTRAL Team's Flag'"
				F.icon_state = "flag_neutral"
			F.info = text("This is an authentic [] flag!\n<font face=vivaldi>Capture the Flag</font>", (src.colour ? src.colour : "neutral"))
			if (src.master.paint_cans)
				var/obj/item/weapon/paint/P = new /obj/item/weapon/paint( L.loc )
				if (src.colour)
					P.colour = src.colour
					P.icon_state = text("paint_[]", src.colour)
				else
					P.colour = "neutral"
					P.icon_state = text("paint_[]", src.colour)
			del(L)
		...
		if (starting)
			for(var/mob/human/H in src.members)
				H.loc = starting.loc
				if ((src.master.autodress && src.colour))
					H.w_uniform = null
					del(H.w_uniform)
					H.shoes = null
					del(H.shoes)
	...
	if ((usr.CanAdmin() || usr == src.captain))
		if (href_list["color"])
			...
			if ((t && src.master.avail_colors.Find(t)))
				...
				src.master.avail_colors += src.colour
				src.colour = t
		if (href_list["base"])
...
/obj/ctf_assist/proc/check_win(O as obj)
	...
	for(var/obj/team/T in src)
		if (text("flag_[]", T.colour) == text("[]", F.icon_state))
			winner = T
		else
	if (winner)
		world << "<H3><B>The game has been won!!!</B></H3>"
		world << text("<B>Team: [] Team led by [] in []</B>", uppertext(winner.colour), winner.captain, winner.base)
...
/obj/ctf_assist/Topic(href, href_list)
	...
	if (href_list["add_team"])
		if (src.avail_bases.len > 0)
			...
			T.colour = pick(src.avail_colors)
			src.avail_bases -= T.base
			src.avail_colors -= T.colour
